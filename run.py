...
def run():
    ...
    # 数据集介绍
    '''
        单被试进行实验(非通用实验), 针对每个被试的每次实验进行分类(15个分类样本)
            SEED 数据集介绍:
            1.实验刺激
    ...
    for i in range(15):
        ...
        for j in range(3):
            ...
            train_label = np.load(os.path.join(
                train_label_root_path, f"subject_{i}/section_{j}_label.npy"))
            ...
            test_label = np.load(os.path.join(
                test_label_root_path, f"subject_{i}/section_{j}_label.npy"))
            ...
            history = model.fit([train_specInput, train_tempInput], train_label, epochs=nbEpoch, batch_size=batch_size,
                                ...
                                callbacks=[early_stopping, save_model], verbose=1)
